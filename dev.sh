...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=buser-req.example.com
...
workon buser-req  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development buser-req npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop buser-req
    docker rm buser-req
    docker run --name buser-req -d --env-file /home/ubuntu/buser-req.env \
...
echo_green "Welcome to buser-req's dev env"
