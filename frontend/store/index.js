  ...
  new Vuex.Store({
    ...
    mutations: {
      ...
      SET_REQS_BOARD(state, reqs) {
        ...
        let board_backlog = reqs["backlog"].filter(
          req => !req.archived && !req.analysis
        );
        board_backlog.sort(compare);
        let board_pending = reqs["pending"].filter(
          req => !req.archived && !req.analysis
        );
        board_pending.sort(compare);
        let board_ongoing = reqs["ongoing"].filter(
          req => !req.archived && !req.analysis
        );
        board_ongoing.sort(compare);
        let board_done = reqs["done"].filter(
          req => !req.archived && !req.analysis
        );
        board_done.sort(compare);
        let board_delivered = reqs["delivered"].filter(
          req => !req.archived && !req.analysis
        );
      ...
      SET_ARCHIVE(state, reqs) {
        ...
        state.reqs_archived = archived_backlog.concat(
          archived_pending,
          archived_ongoing,
          archived_done,
          archived_delivered
      ...
      SET_ANALYSIS(state, reqs) {
        ...
        state.reqs_analysis = analysis_backlog.concat(
          analysis_pending,
          analysis_ongoing,
          analysis_done,
          analysis_delivered
      ...
      ARCHIVE_REQ(state, params) {
        ...
        state.reqs_board[status] = state.reqs_board[status].filter(
          r => r.id !== id
        );
      },
