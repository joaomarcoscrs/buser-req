<template>
  <div>
    <v-card class="card-analise" height="250" width="400" outlined>
      <div class="flexbox-card-analise">
        <div class="titulo-analise">
          <h2 class="font-weight-regular" style="color: #5b5b5b">
            {{ req.title }}
          </h2>
          <v-btn icon small fab class="tirar-margens-do-botao"
            ><img class="icone-slack" src="~/static/slack.png"
          /></v-btn>
        </div>
        ...
        <div class="baixo-analise">
            ...
            <div class="d-flex align-end">
              <template v-if="req.category == 'compra'">
                <v-icon
                  medium
                  color="#FF3677"
                  class="padding-dentro-card-analise"
                  >mdi-cart</v-icon
                >
              </template>
            ...
            <div class="d-flex align-end">
              <template>
                <div class="flex-box-nome-icone">
                  ...
                  >
                  <span
                    class="texto-icone padding-dentro-card font-weight-medium"
                    :class="req.team"
                    >{{ req.team }}</span
                  >
                </div>
          ...
          <div class="texto-descricao descricao-card-analise">
            <h2 class="font-weight-bold">Descrição:</h2>
            <h2 class="font-weight-regular">{{ req.description }}</h2>
          </div>
        </div>
        <v-divider></v-divider>
        <div class="flex-box-botoes" style="background-color: #ececec">
          <v-btn
            @click="handler(req.id)"
            icon
            fab
            class="padding-dentro-card-analise"
            ><v-icon medium color="#64A338">mdi-check</v-icon></v-btn
          >
          <h1 class="creator-card font-weight-regular">
            {{ req.creator }}
          </h1>
          <v-btn
            @click="delete_req(req.id)"
            icon
            fab
            class="padding-dentro-card-analise"
            ><v-icon medium color="#E03B24">mdi-window-close</v-icon></v-btn
          >
        </div>
      </div>
    </v-card>
