    ...
    <v-card 
        ...
        <v-container class="overflow-vert box-alinhar-card-botao" style="min-height: 550px; max-height: 750px">
            <v-row class="width-total">
                <draggable v-model="reqs_board[status]"
                     ...
                     @start="drag = true"
                     @end="drag = false">
                     <reqcard v-for="req in $options.filters.filtro_card(reqs_board[status],'team','operações')" :key="req.id" :req="req"></reqcard>
                </draggable>
                <div key="footer" slot="footer" role="group" class="flex-add">
                  <v-btn text raised block depressed color="#ECECEC" class="teste-botao" @click="add(status)">Adicionar card</v-btn>
                </div>
            </v-row>
        </v-container>
    </v-card>
